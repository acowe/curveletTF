...
int CpxNumTnsBlkd::scatter(BolNumTns& newexists)
{
  ...
  for(int k=0; k<g(); k++)	 for(int j=0; j<f(); j++)		for(int i=0; i<e(); i++) {
    ...
    int div_gbl = glbstts(i,j,k)/blksz, div_lcl = lclsum/blksz;
		lclbid.push_back(div_lcl);
		glbbid.push_back(div_gbl);
		lclsum += _sizes(i,j,k);
	 }
  }
  

  IS lclis;  iC( ISCreateBlock(PETSC_COMM_SELF,  blksz, lclbid.size(), &(lclbid[0]), PETSC_COPY_VALUES, &lclis) );
  IS glbis;  iC( ISCreateBlock(PETSC_COMM_WORLD, blksz, glbbid.size(), &(glbbid[0]), PETSC_COPY_VALUES, &glbis) );
  //lclbid.clear();
  //glbbid.clear();
  ...
  //VecGetSize(glbvec, &xlen);
  //ISGetMinMax(glbis, &min, &max);
  /*
  ...
  */
  ...
  Vec glbvec; iC(VecCreateFromOptions(PETSC_COMM_WORLD, "", blksz, glblszs[mpirank()],  PETSC_DETERMINE, &glbvec)) //iC( VecCreateMPI(PETSC_COMM_WORLD, glblszs[mpirank()], PETSC_DETERMINE, &glbvec) );
  ...
  Vec lclvec;  iC( VecCreateFromOptions(PETSC_COMM_SELF, "", blksz, lclsum, PETSC_DETERMINE, &lclvec) );  //cerr<<"hi,"<< lclsum <<endl;
 
  //3. vec scatter
  VecScatter sc;  iC( VecScatterCreate(glbvec, glbis, lclvec, lclis, &sc) );
  iC( ISDestroy(&lclis) );  iC( ISDestroy(&glbis) ); //SAVE SPACE
  
  iC( VecScatterBegin(sc, glbvec, lclvec, INSERT_VALUES, SCATTER_FORWARD) );
  iC( VecScatterEnd(sc,  glbvec, lclvec, INSERT_VALUES, SCATTER_FORWARD) );
...
int CpxNumTnsBlkd::combine()
{
  ...
  //if (lclbid.size() > 0) cerr<<lclbid[1]<<endl;
  //if (glbbid.size() > 0) cerr<< glbbid[1]<<endl;
  //cerr<<blksz<<","<<lclbid.size()<<","<<glbbid.size()<<endl;
  ...
  */  
  ...
  PetscInt min, max, xlen, glbsize;
  ISGetMinMax(glbis, &min, &max);
  VecGetSize(glbvec, &xlen);
  ISGetSize(glbis, &glbsize);
  

 //cerr<<mpirank()<<","<<min<<","<<max<<","<<xlen<<","<<glbsize<<","<<endl;
 
  //3. vec scatter
  VecScatter sc;  iC( VecScatterCreate(glbvec, glbis, lclvec, lclis, &sc) );
  iC( ISDestroy(&lclis) );  iC( ISDestroy(&glbis) ); //SAVE SPACE
  
  iC( VecScatterBegin(sc,lclvec, glbvec, ADD_VALUES, SCATTER_REVERSE) );
  iC( VecScatterEnd( sc,lclvec, glbvec, ADD_VALUES, SCATTER_REVERSE) );
