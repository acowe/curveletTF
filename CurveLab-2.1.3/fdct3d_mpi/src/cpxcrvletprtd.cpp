...
int CpxCrvletPrtd::scatter(vector< vector<bool> >& newexists)
{
  ...
  IS glbis;  iC( ISCreateGeneral(PETSC_COMM_WORLD, l2gmap.size(), &(l2gmap[0]), PETSC_COPY_VALUES, &glbis) );
...
int CpxCrvletPrtd::combine()
{
  ...
  iC( VecScatterBegin(sc, glbvec, lclvec, ADD_VALUES, SCATTER_REVERSE) );
  iC( VecScatterEnd(  sc, glbvec, lclvec, ADD_VALUES, SCATTER_REVERSE) );
